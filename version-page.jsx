...
function VersionPage() {
  ...
  const versions = [
    ...
    { id: 'v6', label: 'v6', msg: 'Setup project scaffold', author: { n: 'Alex Chen', i: 'AC', c: 'oklch(0.6 0.17 30)' }, when: '1 week ago', branch: 'main', additions: 240, deletions: 0 },
  ];
  ...
  return (
    <div className="page fade-in" style={{ padding: 0, maxWidth: 'none' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '380px 1fr', gap: 0, padding: '0 32px 32px', minHeight: 'calc(100vh - 240px)' }}>
        {/* Timeline */}
        <aside style={{ paddingRight: 24, borderRight: '1px solid var(--line)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
            <div className="eyebrow">Timeline</div>
            {compare && (
              <button className="btn btn-ghost btn-sm" onClick={()=>setCompare(null)}>
                <Icons.x size={11}/> Exit compare
              </button>
            )}
          </div>
          <div className="branch-line" style={{ paddingLeft: 0 }}>
            {versions.map((v, i) => {
              ...
              return (
                <div key={v.id} style={{ position: 'relative', paddingLeft: 28, paddingBottom: 18 }}>
                  ...
                  <div className={`branch-node ${isSel ? 'current' : ''}`} style={{
                    background: isSel ? 'var(--accent)' : isCmp ? 'oklch(0.7 0.15 75)' : 'var(--bg-elev)',
                    borderColor: isSel ? 'var(--accent)' : isCmp ? 'oklch(0.7 0.15 75)' : 'var(--line)',
                  }} />
                  <div
                  ...
                  >
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        ...
                        <span className="chip" style={{ fontSize: 10 }}><Icons.branch size={9}/> {v.branch}</span>
                      </div>
                    ...
                    {isSel && !compare && (
                      <button className="btn btn-secondary btn-sm press" onClick={(e) => { e.stopPropagation(); setCompare(versions[versions.indexOf(v) + 1]?.id); }} style={{ marginTop: 10, width: '100%' }}>
                        <Icons.diff size={11}/> Compare with previous
                      </button>
                    )}
                  </div>
                </div>
        ...
        <main style={{ padding: '0 0 0 28px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 18 }}>
            <div>
              ...
              <div style={{ fontSize: 12.5, color: 'var(--ink-3)', marginTop: 6, display: 'flex', alignItems: 'center', gap: 14 }}>
                ...
                <span>{sel.when}</span>
                <span><Icons.branch size={11} style={{ verticalAlign: 'middle' }}/> {sel.branch}</span>
          ...
          <div style={{ display: 'flex', gap: 4, marginBottom: 14, borderBottom: '1px solid var(--line)' }}>
            {[
              { id: 'content', label: 'Content diff', icon: Icons.text },
              { id: 'visual', label: 'Visual diff', icon: Icons.eye },
              { id: 'files', label: 'Files changed', icon: Icons.layers },
            ].map(t => {
              const I = t.icon;
              return (
          ...
          {tab === 'content' && (
            <div className="card" style={{ overflow: 'hidden' }}>
              <div style={{ padding: '12px 18px', borderBottom: '1px solid var(--line)', display: 'flex', alignItems: 'center', gap: 12, background: 'var(--bg-sunk)' }}>
                <Icons.text size={13} style={{ color: 'var(--ink-3)' }} />
                <span className="mono" style={{ fontSize: 12, color: 'var(--ink-2)' }}>pages/index.html · Hero section</span>
                <span style={{ marginLeft: 'auto', fontSize: 11, color: 'var(--ink-3)' }} className="mono">3 changes</span>
              </div>
              <div style={{ padding: 24, fontSize: 14, lineHeight: 1.7 }}>
                ...
                <div style={{ marginBottom: 16 }}>
                  ...
                  <p>
                    A small <span className="diff-add">design</span> studio in Brooklyn making{' '}
                ...
                <div>
                  ...
                  <p>
                    <span className="diff-del">View portfolio</span>
                    {' '}
                    <span className="diff-add">See recent work</span>
                  </p>
                </div>
              </div>
            </div>
          ...
          {tab === 'visual' && (
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              {[{ l: 'Before · v11', acc: 'var(--err-soft)' }, { l: 'After · v12', acc: 'var(--ok-soft)' }].map((s, i) => (
                <div key={s.l} className="card" style={{ overflow: 'hidden' }}>
                  <div style={{ padding: '10px 14px', borderBottom: '1px solid var(--line)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: s.acc }}>
                    <span style={{ fontSize: 12, fontWeight: 500 }}>{s.l}</span>
                    <span className="chip" style={{ fontSize: 10 }}>1440×900</span>
                  </div>
                  <div className="canvas-dots" style={{ padding: 24, minHeight: 280 }}>
                    <div style={{ background: 'var(--bg)', padding: 32, borderRadius: 8, boxShadow: 'var(--shadow-md)' }}>
                      <div className="eyebrow" style={{ marginBottom: 10 }}>STUDIO HALCYON</div>
                      <h3 style={{ fontFamily: 'var(--font-serif)', fontSize: 20, lineHeight: 1.2, letterSpacing: '-0.02em' }}>
                        {i === 0 ? 'Quietly making sites for brands' : 'Quietly ambitious work for brands that mean it.'}
                      </h3>
                      <p style={{ fontSize: 12, color: 'var(--ink-2)', marginTop: 8 }}>
                        {i === 0 ? 'A small studio in Brooklyn making websites.' : 'A small design studio in Brooklyn making identities, sites, and product UI for the next decade.'}
                      </p>
                      <button className="btn btn-primary btn-sm" style={{ marginTop: 14 }}>
                        {i === 0 ? 'View portfolio' : 'See recent work'} <Icons.arrowRight size={11}/>
                      </button>
                    </div>
                  </div>
                </div>
...
window.VersionPage = VersionPage;

/* global React, Icons, Sparkline, BarChart, COLLAB_PEOPLE */
    ...
    <div className="page fade-in">
      ...
      <div className="card" style={{ padding: 22, marginBottom: 16 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 14 }}>
          ...
          <div className="seg">
            {['visitors','views','time','bounce'].map(m => <button key={m} className={metric===m?'active':''} onClick={()=>setMetric(m)} style={{ textTransform: 'capitalize' }}>{m}</button>)}
          </div>
        </div>
        <div className="big-chart"><LineChart data={visitorsData} height={240} /></div>
      </div>

      {/* Sources + devices + realtime */}
      <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr 1fr', gap: 12, marginBottom: 16 }}>
        ...
        <div className="card" style={{ padding: 22 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 14 }}>
            <Donut segments={devices} size={170} />
          </div>
          {devices.map(d => (
                ...
                <tr key={p.path}>
                  ...
                  <td>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <div className="progress" style={{ width: 50 }}><div style={{ width: `${p.bounce}%`, background: p.bounce > 50 ? 'var(--err)' : 'var(--accent)' }} /></div>

...
export default function SettingsPage({ user }) {
  ...
  const updatePassword = async () => {
    ...
    else { setPwMsg('Password updated successfully'); setCurrentPw(''); setNewPw('') }
  }
        ...
        <aside className="settings-tabs">
          <div className="vtabs">
            {tabs.map(t => {
              const I = t.icon
              return (
                <div key={t.id} className={`vtab ${tab === t.id ? 'active' : ''}`} onClick={() => setTab(t.id)}
                  style={t.danger && tab !== t.id ? { color: 'var(--err)' } : {}}>
                  <I size={14} /> {t.label}
                </div>
              )
            })}
          ...
          {tab === 'profile' && (
            <Section title="Your profile" desc="Visible to teammates and on your published sites.">
              <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 20 }}>
                <div className="avatar" style={{ width: 64, height: 64, fontSize: 22, background: 'oklch(0.6 0.17 30)' }}>
                  {user?.initials || 'AC'}
                </div>
                <div>
              ...
              <label className="field"><span>Full name</span><input className="input" defaultValue={user?.name || 'Alex Chen'} /></label>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <label className="field"><span>Email</span><input className="input" type="email" defaultValue={user?.email || ''} /></label>
              ...
              <label className="field"><span>Bio</span>
                <textarea className="textarea" rows={3} defaultValue="Brooklyn-based designer running a small studio. Currently obsessed with editorial typography on the web." />
              </label>
              <button className="btn btn-primary press" style={{ marginTop: 4 }}>Save changes</button>
            </Section>
          )}
            ...
            <>
              <Section title="Members" desc="3 of 5 seats used on the Pro plan.">
                {COLLAB_PEOPLE.map((p, i) => (
                  <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '12px 0', borderBottom: i < COLLAB_PEOPLE.length - 1 ? '1px solid var(--line)' : 'none' }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: 11.5, color: 'var(--ink-3)' }}>{['alex@halcyon.studio', 'maya@halcyon.studio', 'sam@halcyon.studio'][i] || `${p.name.toLowerCase().replace(' ', '.')}@halcyon.studio`}</div>
                    </div>
                    <select className="input" defaultValue={['owner', 'editor', 'viewer'][i] || 'viewer'} style={{ width: 110, height: 30, fontSize: 12 }}>
              ...
              <Section title="Your domains" desc={`${OWNED_DOMAINS.length} domains registered`}>
                {OWNED_DOMAINS.map((d, i) => (
                  <div key={d.name} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 0', borderBottom: i < OWNED_DOMAINS.length - 1 ? '1px solid var(--line)' : 'none' }}>
                    <div style={{ width: 36, height: 36, borderRadius: 9, background: d.status === 'expiring' ? 'var(--warn-soft)' : 'var(--accent-soft)', color: d.status === 'expiring' ? 'var(--warn)' : 'var(--accent-ink)', display: 'grid', placeItems: 'center', flexShrink: 0 }}>
                      <Icons.globe size={16} />
                    </div>
                    <div style={{ flex: 1 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          {tab === 'security' && (
            <>
              <Section title="Change password">
                <label className="field"><span>Current password</span><input className="input" type="password" value={currentPw} onChange={e => setCurrentPw(e.target.value)} placeholder="••••••••" /></label>
                <label className="field"><span>New password</span><input className="input" type="password" value={newPw} onChange={e => setNewPw(e.target.value)} placeholder="At least 8 characters" /></label>
                {pwMsg && <div style={{ fontSize: 12.5, color: pwMsg.includes('success') ? 'var(--ok)' : 'var(--err)', marginBottom: 8 }}>{pwMsg}</div>}
                <button className="btn btn-primary press" onClick={updatePassword} disabled={pwSaving}>
          ...
          {tab === 'danger' && (
            <Section title="Danger zone" desc="Irreversible actions. Read carefully before proceeding.">
              {[
                ...
                { l: 'Delete workspace',      s: 'Permanently delete this workspace and all sites. Cannot be undone.', cta: 'Delete workspace', d: true },
              ].map((a, i, arr) => (

  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', flex: 1, minHeight: 0, height: '100%' }}>
      ...
      <header className="topbar" style={{ paddingLeft: 16, flexShrink: 0 }}>
        ...
        <button className="btn btn-secondary btn-sm press" onClick={() => setPreviewOpen(true)}><Icons.eye size={13} /> Preview</button>
        <button className="btn btn-primary btn-sm press" onClick={onPublish}>{site.status === 'published' ? 'Update' : 'Publish'} <Icons.arrowRight size={12} /></button>
      </header>
        ...
        <div className="editor-panel">
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                  {site.content.sections.map((s, i) => (
                    <div key={s.id} onClick={() => setActive(s.id)}
                      style={{ padding: '10px 12px', borderRadius: 8, cursor: 'pointer', background: active === s.id ? 'var(--accent-soft)' : 'var(--bg-sunk)', border: `1px solid ${active === s.id ? 'oklch(0.55 0.18 268 / 0.4)' : 'var(--line)'}`, transition: 'all .15s var(--ease-snap)', display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          {tab === 'design' && (
            <div className="editor-section">
              ...
              <select className="input" defaultValue="inter">
                <option value="inter">Inter Tight + JetBrains Mono</option>
                <option value="sohne">Söhne + Söhne Mono</option>
                <option value="tiempos">Tiempos + Söhne</option>
              </select>
            </div>
          )}
        </div>
      ...
      {previewOpen && (
        <div className="modal-overlay" onClick={() => setPreviewOpen(false)}>
          <div style={{ width: '90vw', maxWidth: 1200, height: '85vh', background: 'white', borderRadius: 16, overflow: 'auto', position: 'relative', boxShadow: 'var(--shadow-lg)' }} onClick={e => e.stopPropagation()}>
            <button className="btn btn-secondary btn-sm" style={{ position: 'absolute', top: 12, right: 12, zIndex: 2 }} onClick={() => setPreviewOpen(false)}>

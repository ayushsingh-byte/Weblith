...
export default function CreateSiteModal({ open, onClose, onCreate }) {
  ...
  const submit = () => {
    ...
    setTimeout(() => {
      setCreating(false)
      onCreate({ name: name || 'Untitled site', slug: slug || 'untitled', template: tmpl })
      setStep(1); setName(''); setSlug(''); setTmpl('portfolio')
    }, 700)
  }
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: 720 }}>
        <div className="modal-header" style={{ paddingBottom: 12 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              ...
              <h3 style={{ fontSize: 18, marginTop: 4 }}>{step === 1 ? 'Choose a starting point' : 'Name your site'}</h3>
            </div>
            <button className="btn btn-icon btn-ghost btn-sm" onClick={onClose}><Icons.x size={14} /></button>
          </div>
        ...
        <div className="modal-body" style={{ maxHeight: 480, overflowY: 'auto' }}>
          ...
          {step === 2 && (
            <div className="fade-in" style={{ paddingTop: 8 }}>
              ...
              <input className="input" value={name} onChange={e => onName(e.target.value)} placeholder="My new site" autoFocus />
              <div style={{ height: 14 }} />
              <label className="field-label">URL</label>
              <div style={{ display: 'flex', alignItems: 'center' }}>
                <span className="mono" style={{ padding: '10px 12px', background: 'var(--bg-sunk)', border: '1px solid var(--line)', borderRight: 'none', borderRadius: '10px 0 0 10px', color: 'var(--ink-3)', fontSize: 12, whiteSpace: 'nowrap' }}>weblith.site /</span>
        ...
        <div className="modal-footer">
          {step === 2 && <button className="btn btn-secondary" onClick={() => setStep(1)}><Icons.arrowLeft size={12} /> Back</button>}
          <span style={{ flex: 1 }} />
          {step === 1
            ? <button className="btn btn-primary" onClick={() => setStep(2)} disabled={!tmpl}>Continue <Icons.arrowRight size={12} /></button>
            : <button className="btn btn-primary" onClick={submit} disabled={!name || creating}>
                {creating ? <><span className="spinner" /> Creating…</> : <>Create site <Icons.arrowRight size={12} /></>}
              </button>
          }
        </div>
      </div>

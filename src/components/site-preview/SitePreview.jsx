function PreviewSection({ section, idx }) {
  if (section.type === 'hero') return (
    <section style={{ padding: '96px 48px 72px', maxWidth: 980, margin: '0 auto' }}>
      <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'oklch(0.62 0.008 80)', marginBottom: 18 }}>— 0{idx + 1} / Hero</div>
      <h1 style={{ fontSize: 56, lineHeight: 1.05, letterSpacing: '-0.035em', fontWeight: 600, marginBottom: 18, textWrap: 'balance', color: 'oklch(0.18 0.008 80)' }}>{section.heading || 'Untitled heading'}</h1>
      <p style={{ fontSize: 19, color: 'oklch(0.42 0.008 80)', lineHeight: 1.5, maxWidth: 620, textWrap: 'pretty' }}>{section.sub}</p>
      {section.cta && (
        <div style={{ marginTop: 32, display: 'flex', gap: 8 }}>
          <button style={{ padding: '12px 18px', borderRadius: 8, background: 'oklch(0.18 0.008 80)', color: 'white', border: 'none', fontWeight: 500, fontSize: 14, cursor: 'pointer' }}>{section.cta} →</button>
          <button style={{ padding: '12px 18px', borderRadius: 8, background: 'transparent', color: 'oklch(0.18 0.008 80)', border: '1px solid oklch(0.84 0.008 80)', fontWeight: 500, fontSize: 14, cursor: 'pointer' }}>Learn more</button>
        </div>
      )}
    </section>
  )
  if (section.type === 'grid') return (
    <section style={{ padding: '48px 48px 72px', maxWidth: 980, margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 24 }}>
        <h2 style={{ fontSize: 28, fontWeight: 600, letterSpacing: '-0.025em', color: 'oklch(0.18 0.008 80)' }}>{section.heading}</h2>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 16 }}>
        {(section.items || []).map((it, i) => (
          <div key={i} style={{ borderRadius: 12, overflow: 'hidden', background: 'oklch(0.965 0.005 80)' }}>
            <div style={{ aspectRatio: '4/3', background: 'repeating-linear-gradient(135deg, oklch(0.93 0.006 80), oklch(0.93 0.006 80) 8px, oklch(0.95 0.006 80) 8px, oklch(0.95 0.006 80) 16px)', display: 'grid', placeItems: 'center', fontFamily: 'var(--font-mono)', fontSize: 11, color: 'oklch(0.62 0.008 80)' }}>project image</div>
            <div style={{ padding: '14px 16px' }}>
              <div style={{ fontWeight: 500, marginBottom: 4, color: 'oklch(0.18 0.008 80)' }}>{it.title}</div>
  ...
  if (section.type === 'text') return (
    <section style={{ padding: '48px 48px 72px', maxWidth: 760, margin: '0 auto' }}>
      <h2 style={{ fontSize: 28, fontWeight: 600, letterSpacing: '-0.025em', marginBottom: 16, color: 'oklch(0.18 0.008 80)' }}>{section.heading}</h2>

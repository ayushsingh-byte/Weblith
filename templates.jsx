/* global React, Icons, TEMPLATES */
const { useState } = React;

function TemplatesPage({ onUse, embedded = false, selected, onSelect }) {
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: 14 }} className="stagger">
        {list.map(t => (
          <div key={t.id}
            ...
            onClick={() => onSelect ? onSelect(t.id) : onUse?.(t.id)}
            style={{
              cursor: 'pointer',
              overflow: 'hidden',
              borderColor: selected === t.id ? 'var(--accent)' : undefined,
              boxShadow: selected === t.id ? '0 0 0 3px oklch(0.55 0.18 268 / 0.15)' : undefined,
            }}>
            ...
            <div style={{ padding: 14 }}>
              ...
              <p style={{ fontSize: 12.5, color: 'var(--ink-2)', lineHeight: 1.45 }}>{t.desc}</p>
            </div>
          </div>
        ))}
      </div>
...
// Stylized inline preview per template
function TemplatePreview({ tmpl }) {
  ...
  if (tmpl.id === 'blank') return (
    <div style={{ position: 'absolute', inset: 0, display: 'grid', placeItems: 'center', color: 'var(--ink-3)', fontFamily: 'var(--font-mono)', fontSize: 11 }}>
      empty canvas
    </div>
  );
  if (tmpl.id === 'portfolio') return (
    <div style={{ position: 'absolute', inset: 18, display: 'flex', flexDirection: 'column', gap: 8 }}>
      ...
      <div style={{ flex: 1, marginTop: 8, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
        <div style={{ background: inkLine, borderRadius: 4 }} />
        <div style={{ background: inkLine, borderRadius: 4 }} />
        <div style={{ background: inkLine, borderRadius: 4 }} />
        <div style={{ background: inkLine, borderRadius: 4 }} />
      </div>
  ...
  if (tmpl.id === 'blog') return (
    <div style={{ position: 'absolute', inset: 22, fontFamily: 'serif' }}>
      ...
      <div style={{ height: 3, width: '100%', background: inkSoft, marginBottom: 3 }} />
      <div style={{ height: 3, width: '100%', background: inkSoft, marginBottom: 3 }} />
      <div style={{ height: 3, width: '70%', background: inkSoft, marginBottom: 8 }} />
      <div style={{ height: 3, width: '100%', background: inkSoft, marginBottom: 3 }} />
      <div style={{ height: 3, width: '50%', background: inkSoft }} />
    </div>
  );

/* global React, Icons, Sparkline, BarChart, CollabStack, Avatar, COLLAB_PEOPLE, ACTIVITY */
const { useState, useEffect } = React;

// 30-day visitor data
const VISITOR_DATA = [
  120, 145, 132, 168, 192, 178, 210, 245, 230, 198,
  220, 265, 280, 312, 298, 340, 358, 322, 380, 412,
  398, 445, 420, 468, 502, 488, 530, 545, 568, 612,
];
...
function Dashboard({ sites, onOpen, onCreate, user, onNavigate, onOpenAI }) {
    ...
    <div className="page fade-in">
      <div className="page-header">
        <div>
          ...
          <h1 className="display" style={{ marginTop: 8 }}>Welcome back, {user.name.split(' ')[0]}.</h1>
        ...
        <aside style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div className="card" style={{ padding: 18 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
              ...
              <Icons.bolt size={12} className="" style={{ color: 'var(--ink-3)' }} />
            </div>
            {sites.filter(s => s.status === 'published').slice(0, 3).map((s, i) => (
              <div key={s.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 0', borderBottom: i < 2 ? '1px solid var(--line)' : 'none', cursor: 'pointer' }} onClick={() => onOpen(s.id)}>
              ...
              {ACTIVITY.slice(0, 5).map((a, i) => (
                <li key={i} style={{ fontSize: 12.5, lineHeight: 1.45, position: 'relative', paddingLeft: 18, paddingBottom: i < 4 ? 14 : 0 }}>
                  ...
                  <div style={{ color: 'var(--ink)' }}>
                    <strong style={{ fontWeight: 500 }}>{a.who}</strong> <span style={{ color: 'var(--ink-2)' }}>{a.what}</span>
                  </div>
      ...
      <div style={{ aspectRatio: '16/10', background: site.color, position: 'relative', borderBottom: '1px solid var(--line)', overflow: 'hidden' }}>
        ...
        {hover && (
          <div className="fade-in" style={{ position: 'absolute', inset: 0, background: 'oklch(0.18 0.008 80 / 0.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, backdropFilter: 'blur(2px)' }}>
            <button className="btn btn-sm" style={{ background: 'var(--bg)', color: 'var(--ink)' }}><Icons.text size={12} /> Edit</button>
            {site.status === 'published' && <button className="btn btn-sm" style={{ background: 'transparent', color: 'var(--bg)', border: '1px solid oklch(0.99 0 0 / 0.4)' }}><Icons.external size={12} /> Visit</button>}
          </div>
        )}
      </div>
...
// ---------- Sites table page (unchanged shape, polished) ----------
function SitesPage({ sites, onOpen, onCreate }) {
              ...
              <tr key={s.id} style={{ cursor: 'pointer' }} onClick={() => onOpen(s.id)}>
                <td>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    ...
                    <div><div style={{ fontWeight: 500 }}>{s.name}</div><div style={{ fontSize: 11.5, color: 'var(--ink-3)', textTransform: 'capitalize' }}>{s.template} template</div></div>
                  </div>
                </td>
                <td><span className={`chip ${s.status === 'published' ? 'chip-ok' : ''}`}><span className={`dot ${s.status === 'draft' ? 'dot-idle' : ''}`} />{s.status}</span></td>
                <td className="mono" style={{ color: 'var(--ink-2)' }}>{s.domain}</td>
                <td style={{ fontWeight: 500 }}>{s.visitors.toLocaleString()}</td>
                <td><Sparkline data={s.status === 'published' ? [3,5,4,7,6,8,9,11,10,13] : [0,0,0,0,0,0,0,0]} width={80} height={22} color={s.status === 'published' ? 'var(--accent)' : 'var(--ink-3)'} /></td>

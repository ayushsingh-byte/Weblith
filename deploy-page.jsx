...
function DeployPage({ sites }) {
  const [siteId, setSiteId] = useState(sites[0]?.id);
  const site = sites.find(s => s.id === siteId);
  const [running, setRunning] = useState(false);
  const [stage, setStage] = useState(3); // pipeline stage
  ...
  const deploy = () => {
    ...
    const lines = [
      ...
      { t: 'ok',   m: '✓ Deploy live · alex-chen.weblith.site' },
    ];
    lines.forEach((l, i) => {
      setTimeout(() => {
        setLog(prev => [...prev, l]);
    ...
    <div className="page fade-in">
      <div className="page-hero">
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <button className="btn btn-accent press" onClick={deploy} disabled={running}>
      ...
      {tab === 'pipeline' && (
        <div className="card" style={{ background: 'oklch(0.16 0.01 270)', color: 'oklch(0.92 0.005 80)', border: 'none', overflow: 'hidden', minHeight: 360 }}>
          ...
          <pre style={{ margin: 0, padding: 18, fontFamily: 'var(--font-mono)', fontSize: 12, minHeight: 320, lineHeight: 1.7, whiteSpace: 'pre-wrap' }}>
            ...
            {running && <span style={{ animation: 'blink 1s infinite' }}>▍</span>}
          </pre>
        </div>
      )}
            ...
            <tbody>
              {deploys.map(d => (
                <tr key={d.id}>
                  ...
                  <td><span className="mono" style={{ fontSize: 11.5, color: 'var(--ink-2)' }}><Icons.branch size={11} style={{ verticalAlign: 'middle' }}/> {d.branch}</span></td>
                  ...
                  <td style={{ textAlign: 'right' }}>
                    {d.status === 'previous' ? <button className="btn btn-ghost btn-sm">Rollback</button> : <button className="btn btn-icon btn-ghost btn-sm"><Icons.more size={13}/></button>}
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <div className="card" style={{ padding: 22 }}>
          ...
          {envs.map((e, i) => (
            <div key={i} className="kv-row">
              ...
              <div style={{ display: 'flex', gap: 4 }}>
                <button className="btn btn-icon btn-ghost btn-sm" title={e.secret ? 'Show' : 'Hide'}><Icons.eye size={13}/></button>
          ...
          <div style={{ borderTop: '1px solid var(--line)', marginTop: 16, paddingTop: 16 }}>
            <div className="kv-row">
              ...
              <button className="btn btn-primary btn-sm press" disabled={!newK} onClick={() => { setEnvs(es => [...es, { k: newK, v: newV, secret: false }]); setNewK(''); setNewV(''); }}><Icons.plus size={12}/> Add</button>
            </div>
          </div>
        </div>
      )}
      ...
      {tab === 'settings' && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          <div className="card" style={{ padding: 22 }}>
            ...
            <label className="field"><span>Framework preset</span><select className="input" defaultValue="static"><option value="static">Static (auto-detect)</option><option>Next.js</option><option>Astro</option><option>Vite</option></select></label>
            ...
            <label className="field"><span>Node version</span><select className="input" defaultValue="20"><option>20</option><option>18</option><option>16</option></select></label>
          </div>
          <div className="card" style={{ padding: 22 }}>

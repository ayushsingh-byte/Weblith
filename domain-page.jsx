...
function DomainPage({ sites }) {
  ...
  const startWizard = () => { setShowWiz(true); setWizStep(1); setWizDomain(''); };
    ...
    <div className="page fade-in">
      <div className="page-hero">
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <button className="btn btn-primary press" onClick={startWizard}><Icons.plus size={13}/> Add domain</button>
        </div>
      </div>

      {/* Default subdomain banner */}
      <div className="card" style={{ padding: 18, marginBottom: 14, display: 'flex', alignItems: 'center', gap: 14 }}>
        <div className="domain-icon" style={{ background: 'var(--accent-soft)', color: 'var(--accent-ink)' }}><Icons.bolt size={16}/></div>
        <div style={{ flex: 1 }}>
        ...
        <div>
          {domains.map(d => (
            <div key={d.name} className="domain-row">
              <div className="domain-icon"><Icons.globe2 size={16} /></div>
              <div>
                ...
                <div style={{ fontSize: 11.5, color: 'var(--ink-3)', marginTop: 4, display: 'flex', gap: 14 }}>
                  {d.status === 'live' ? (
                    <>
                      <span style={{ display: 'inline-flex', gap: 4, alignItems: 'center' }}><Icons.lock size={10}/> SSL · {d.ssl}</span>
                      <span>Issued {d.issued}</span>
                      <span>Renews {d.expires}</span>
                    </>
              ...
              <div style={{ display: 'flex', gap: 4 }}>
                <button className="btn btn-icon btn-ghost btn-sm" title="Visit"><Icons.external size={13}/></button>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        <div className="card" style={{ padding: 22 }}>
          ...
          {[
            ...
            { l: 'WWW redirect', s: 'www.studio-halcyon.com → studio-halcyon.com', on: true },
          ].map((x, i) => (
            ...
            <div className="modal-body">
              ...
              {wizStep === 1 && (
                <>
                  ...
                  <p style={{ fontSize: 12.5, color: 'var(--ink-2)', marginBottom: 16 }}>Use a domain you've already purchased from any registrar (Namecheap, Google, Cloudflare, etc.)</p>
                  <input className="input" placeholder="example.com" value={wizDomain} onChange={e => setWizDomain(e.target.value)} autoFocus />
                  <button className="btn btn-primary press" disabled={!wizDomain} onClick={() => setWizStep(2)} style={{ width: '100%', marginTop: 14 }}>Continue <Icons.arrowRight size={13}/></button>
                </>
              )}
